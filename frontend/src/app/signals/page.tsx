...
import { Zap, TrendingUp, Sparkles, TrendingDown, LayoutGrid, List, RefreshCw } from "lucide-react";
...
export default function SignalsPage() {
  ...
  async function loadSignals() {
    setLoading(true);
    try {
      // Real signals only — no demo fallback.
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredAndSortedSignals.map((signal) => (
              <SignalCard key={signal.id} signal={signal} variant={viewMode} />
            ))}
          </div>
        )}
